...
`crypto_exchange_adapters.coinbase_rest.CoinbaseMarketTradesClient` performs
bounded Advanced Trade market-trade reads with capped retries, rate-limit
backoff, recursive full-window splitting, strict response validation, and
normalization to the shared `ExchangeTrade` model.
