...
def _objects(client, prefix: str) -> list[dict[str, object]]:
    return client.list_objects_v2(Bucket="crypto-data", Prefix=prefix).get("Contents", [])
...
def test_dashboard_reads_postgres_and_minio_without_mutating_state(tmp_path: Path) -> None:
    import boto3
    import psycopg
    ...
    client = boto3.client(
        ...
        aws_secret_access_key=os.getenv("OPERATOR_DASHBOARD_S3_SECRET_KEY", "minioadmin"),
    ...
    plan_body = _body({"name": "integration-dashboard", "start_not_before": "2026-09-07T00:00:00Z"})
    ...
    try:
        ...
        with psycopg.connect(database_url) as connection:
            ...
            before_pilots = connection.execute("SELECT count(*) FROM paper_pilots").fetchone()[0]
            before_sessions = connection.execute("SELECT count(*) FROM paper_sessions").fetchone()[0]
        ...
        assert snapshot["trusted_data"]["raw_integrity"]["details"]["Kafka records"] == "3"
        assert snapshot["trusted_data"]["curated_trades"]["details"]["Logical trades"] == "3"
        assert snapshot["trusted_data"]["one_minute_candles"]["details"]["Candles"] == "2"
        assert snapshot["research"]["oos"]["candidate"] == candidate
        assert snapshot["draft_plan"]["raw_sha256"] == hashlib.sha256(plan_body).hexdigest()
        if before_pilots == 0:
        ...
        with psycopg.connect(database_url) as connection:
            connection.execute("BEGIN READ ONLY")
            assert connection.execute("SELECT count(*) FROM paper_pilots").fetchone()[0] == before_pilots
            assert connection.execute("SELECT count(*) FROM paper_sessions").fetchone()[0] == before_sessions
    finally:
